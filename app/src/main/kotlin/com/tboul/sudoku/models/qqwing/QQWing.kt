...
class QQWing {
     ...
     */
    private val solutionRound = IntArray(BOARD_SIZE)
     ...
     * possibility.
     */
    private val possibilities = IntArray(POSSIBILITY_SIZE)
     ...
     */
    private val randomBoardArray = fillIncrementing(IntArray(BOARD_SIZE))
     ...
     */
    private val randomPossibilityArray = fillIncrementing(IntArray(ROW_COL_SEC_SIZE))
    ...
    val difficulty: Difficulty
        get() {
            if (guessCount > 0) return Difficulty.EXPERT
            if (boxLineReductionCount > 0) return Difficulty.INTERMEDIATE
            if (pointingPairTripleCount > 0) return Difficulty.INTERMEDIATE
            if (hiddenPairCount > 0) return Difficulty.INTERMEDIATE
            if (nakedPairCount > 0) return Difficulty.INTERMEDIATE
            if (hiddenSingleCount > 0) return Difficulty.EASY
            return if (singleCount > 0) Difficulty.SIMPLE else Difficulty.UNKNOWN
        }

    /**
     * Get the difficulty rating.
     */
    val difficultyAsString: String
        get() = difficulty.getName()
     ...
     */
    val singleCount: Int
        get() = getLogCount(solveInstructions, LogType.SINGLE)
     ...
     * section.
     */
    val hiddenSingleCount: Int
        get() = getLogCount(solveInstructions, LogType.HIDDEN_SINGLE_ROW) +
     ...
     * this puzzle.
     */
    val nakedPairCount: Int
        get() = getLogCount(solveInstructions, LogType.NAKED_PAIR_ROW) +
     ...
     * this puzzle.
     */
    val hiddenPairCount: Int
        get() = getLogCount(solveInstructions, LogType.HIDDEN_PAIR_ROW) +
     ...
     * solving this puzzle.
     */
    val pointingPairTripleCount: Int
        get() = getLogCount(solveInstructions, LogType.POINTING_PAIR_TRIPLE_ROW) + getLogCount(
            solveInstructions,
     ...
     * puzzle.
     */
    val boxLineReductionCount: Int
        get() = getLogCount(solveInstructions, LogType.ROW_BOX) + getLogCount(
            solveInstructions,
            LogType.COLUMN_BOX
        )
     ...
     */
    val guessCount: Int
        get() = getLogCount(solveInstructions, LogType.GUESS)
    ...
    val backtrackCount: Int
        get() = getLogCount(solveHistory, LogType.ROLLBACK)

    val solveInstructionsString: String
        get() = if (isSolved) {
    ...
    val solveHistoryString: String
        get() = historyToString(solveHistory)
    ...
    val puzzleString: String
        get() = puzzleToString(puzzle)

    val solutionString: String
        get() = puzzleToString(solution)
    ...
    fun setPuzzle(initPuzzle: IntArray?): Boolean {
        for (i in 0 until BOARD_SIZE) {
            puzzle[i] = if (initPuzzle == null) 0 else initPuzzle[i]
        }
        return reset()
    }
    ...
    companion object {
        ...
        const val ROW_COL_SEC_SIZE = GRID_SIZE * GRID_SIZE

        const val SEC_GROUP_SIZE = ROW_COL_SEC_SIZE * GRID_SIZE

        const val BOARD_SIZE = ROW_COL_SEC_SIZE * ROW_COL_SEC_SIZE

        const val POSSIBILITY_SIZE = BOARD_SIZE * ROW_COL_SEC_SIZE
        ...
        private// not the first and last value which are NONE and RANDOM
         ...
         */
        internal fun cellToSectionStartCell(cell: Int): Int {
         ...
         */
        internal fun rowToFirstCell(row: Int): Int {
            return 9 * row
        }
         ...
         */
        internal fun columnToFirstCell(column: Int): Int {
            return column
        }
         ...
         */
        internal fun sectionToFirstCell(section: Int): Int {
         ...
         */
        internal fun getPossibilityIndex(valueIndex: Int, cell: Int): Int {
         ...
         */
        internal fun rowColumnToCell(row: Int, column: Int): Int {
         ...
         * cell (0-80)
         */
        internal fun sectionToCell(section: Int, offset: Int): Int {
